...
class LampNode extends DeviceNode {
    constructor(config) {
        super(config);
        console.log("LampNode[" + this.logid() + "].constructor()");
        this.button = null;
        this.state = { on: false, brightness: 0 };
    }

    onStartup() {
        console.log("LampNode[" + this.logid() + "].onStarted()");
        var instance = this;
    ...
    onLightUpdate(event) {
        console.log("LampNode[" + this.logid() + "].onLightUpdate()");
