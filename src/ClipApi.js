...
class ClipApi extends events.EventEmitter {
    ...
    getSortedDeviceServices(uuid,rtype="device") {
        console.log("ClipApi[" + this._name + "].getSortedDeviceServices(" + uuid + "," + rtype + ")");

        var services = [];

        var providers = this._servicesByTypeAndId[rtype];
        if (providers) {
